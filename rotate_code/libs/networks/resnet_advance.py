...
def resnet_base(img_batch, scope_name, is_training=True):
    ...
    _, feat_dict = get_resnet_fn(input_x=img_batch, scope=scope_name,
    ...
    for level in range(5, 1, -1):
        add_heatmap(feat_dict['C%d' % level], name='Layer%d/C%d_heat' % (level, level))
