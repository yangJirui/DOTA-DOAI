...
def enrich_semantics(net, channels, num_layer, scope):
    with tf.variable_scope(scope):
...
def resnet_base(img_batch, scope_name, is_training=True):
    ...
    with tf.variable_scope("enrich_semantics"):
        with slim.arg_scope([slim.conv2d], weights_regularizer=slim.l2_regularizer(cfgs.WEIGHT_DECAY),
                             normalizer_fn=None):
            pyramid_dict["P2"] = enrich_semantics(net=pyramid_dict["P2"],
                                                  num_layer=4,
                                                  channels=256, scope="enrich_p2")
            pyramid_dict["P3"] = enrich_semantics(net=pyramid_dict["P3"],
                                                  num_layer=3,
                                                  channels=256, scope="enrich_p3")
    with tf.variable_scope("enlarge_RF"):
        with slim.arg_scope([slim.conv2d], weights_regularizer=slim.l2_regularizer(cfgs.WEIGHT_DECAY),
            ...
            pyramid_dict["P5"] = slim.conv2d(pyramid_dict["P5"],
                                             num_outputs=256, kernel_size=[7, 7], stride=1, rate=2)
