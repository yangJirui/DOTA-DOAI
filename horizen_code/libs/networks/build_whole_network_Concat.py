        ...
        with tf.name_scope('postprocess_fastrcnn'):
            ...
            for i in range(1, cfgs.CLASS_NUM + 1):
                ...
                if cfgs.SOFT_NMS:
                    ...
                    det = tf.concat([tmp_decoded_boxes, tf.reshape(tmp_score, [-1, 1])],
                                    axis=1)
                    keep = tf.py_func(nms_wrapper.soft_nms_cpu,
                                      inp=[det, 0.5, cfgs.FAST_RCNN_NMS_IOU_THRESHOLD,
                                           0.001, cfgs.FAST_RCNN_NMS_MAX_BOXES_PER_CLASS],
                                      Tout=tf.int64)
