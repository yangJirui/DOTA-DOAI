...
def get_restorer():
    COCO_path = '/home/omnisky/TF_pretrained/yangxue_coco/voc_699999model.ckpt'
    ...
    print ("COCO path is : %s" % COCO_path)
    model_variables = slim.get_model_variables()
    restore_list = []
    for var in model_variables:
        if var.name.startswith((cfgs.NET_NAME, 'build_pyramid')):
            restore_list.append(var)
    restore_variables = restore_list
    for var in restore_variables:
        print (var.op.name)
    restorer = tf.train.Saver(restore_variables)
    print(20 * "****")
    print("restore from pretrained_weighs in COCO...")
    return restorer, COCO_path
