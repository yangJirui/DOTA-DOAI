...
from libs.networks import resnet_advance
...
class DetectionNetwork(object):
    ...
    def build_base_network(self, input_img_batch):
        ...
        if self.base_network_name.startswith('resnet_v1'):
            ...
            return resnet_advance.resnet_base(input_img_batch, scope_name=self.base_network_name,
    ...
    def postprocess_fastrcnn(self, rois, bbox_ppred, scores, img_shape):
        ...
        with tf.name_scope('postprocess_fastrcnn'):
            ...
            for i in range(1, cfgs.CLASS_NUM+1):
                ...
                #                'soccer-ball-field': 0.3, 'small-vehicle': 0.2, 'ship': 0.1, 'plane': 0.3,
                #                'large-vehicle': 0.15, 'helicopter': 0.2, 'harbor': 0.1, 'ground-track-field': 0.3,
                ...
                if cfgs.SOFT_NMS:
                    ...
                    keep = nms_wrapper.nms_rotate(decode_boxes=tmp_decoded_boxes,
                                                  scores=tmp_score,
                                                  iou_threshold=r_threshold[LABEl_NAME_MAP[i]],  # cfgs.FAST_RCNN_NMS_IOU_THRESHOLD,
        ...
        with tf.variable_scope('build_loss') as sc:
            ...
            if cfgs.USE_SUPERVISED_MASK:
                with tf.variable_scope("supervised_mask_loss"):
                    ...
                    mask_losses = mask_losses * cfgs.SUPERVISED_MASK_LOSS_WEIGHT / float(len(mask_list))
                    # mask_losses = tf.Print(mask_losses, [mask_losses], summarize=10, message="mask_loss")
            ...
            with tf.variable_scope('FastRCNN_loss'):
                if not cfgs.FAST_RCNN_MINIBATCH_SIZE == -1:
                    bbox_loss = losses.smooth_l1_loss_rcnn_r(bbox_pred=bbox_pred,
                                                             bbox_targets=bbox_targets,
                                                             label=labels,
